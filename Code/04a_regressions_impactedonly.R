...
# Prepare wide data
mean_data_impacted <- monitor_satellite_long_impacted %>%
...
# Cluster variable (monitor location)
cluster_var <- interaction(mean_data_impacted$x, mean_data_impacted$y)

# Difference-in-means test: Updated - Original
model_diff <- lm(diff ~ 1, data = mean_data_impacted)
...
# Mean and SE for each group (non-clustered SEs shown)
mean_orig <- mean(mean_data_impacted$pm_orig, na.rm = TRUE)
mean_updt <- mean(mean_data_impacted$pm_updt, na.rm = TRUE)
se_orig <- sd(mean_data_impacted$pm_orig, na.rm = TRUE) / sqrt(nrow(mean_data_impacted))
se_updt <- sd(mean_data_impacted$pm_updt, na.rm = TRUE) / sqrt(nrow(mean_data_impacted))
...
# Set output location
output_file_means_impacted <- file.path(output_dir, "means_table_impacted.tex")

# Write and output LaTeX table
cat(sprintf('
\\begin{table}[!htbp] \\centering
  \\caption{Mean PM\\textsubscript{2.5} Before and After EPA Correction (Impacted Monitors Only)}
  \\label{tab:means_impacted}
...
\\hline
\\hline \\\\[-1.8ex]
\\multicolumn{4}{p{\\textwidth}}{\\textit{Notes:} This table compares the average PM\\textsubscript{2.5} measured by monitors before and after the EPA correction, restricted to impacted (originally erroneous) monitors. Column (3) shows the difference in means with robust standard errors clustered by monitor location. Significance levels: \\$^{*}p < 0.1\\$, \\$^{**}p < 0.05\\$, \\$^{***}p < 0.01\\$. Source: The EPA\\textquotesingle s Air Quality System.}
\\end{tabular*}
\\end{table}
...
# Print confirmation
message("LaTeX table written to: ", output_file_means_impacted)


###### SUR Regression and Wald Test and Output LaTeX Table ######
...
# ----- SUR Regression and Wald Test ------  
 ...
 # ------ LaTeX Table ------ 
  ...
  # Set output location
  output_file_regression_impacted <- file.path(output_dir, "regression_table_impacted.tex")
